...
export default function WinnersReveal({ label, winners }: { label: string; winners: Entry[] }) {
  ...
  return (
    <div>
      <div className="text-center font-mono text-[11px] uppercase tracking-[0.3em] text-ember">
        {label} category
      </div>

      {!opened ? (
        <div className="mt-4 rounded-lg border border-dashed border-ember/25 bg-panel/40 px-5 py-14 text-center">
          <div className="font-mono text-[11px] uppercase tracking-[0.35em] text-dim">
            results sealed
          </div>
          <p className="mx-auto mt-3 max-w-sm text-sm text-dim">
            {solo ? "The winner is" : `${winners.length} winners are`} locked in. Press when
            you&apos;re ready.
          </p>
          <button
            onClick={reveal}
            className="seal-btn mt-7 rounded-full border border-ember/60 bg-ember/10 px-9 py-4 font-mono text-xs uppercase tracking-[0.3em] text-ember transition hover:bg-ember/20"
          >
            ★ Reveal the {solo ? "winner" : "winners"}
          </button>
        </div>
      ) : (
        <div className={`mt-6 grid gap-4 ${solo ? "mx-auto max-w-2xl" : "md:grid-cols-3"}`}>
          {winners.map((e, i) => {
            ...
            return (
              <Link
                ...
                className={`group overflow-hidden rounded-lg border bg-panel transition hover:border-photon/60 ${
                  isFirst && !solo ? "border-ember/50 md:-translate-y-2" : isFirst ? "border-ember/50" : "border-white/10"
              ...
              >
                <div className="flex items-center justify-between border-b border-white/5 px-3 py-2 font-mono text-[10px] tracking-widest">
                  <span className="text-ember">
                    {e.winner === "1" ? (solo ? "★ WINNER" : "★ FIRST") : e.winner === "2" ? "SECOND" : "THIRD"}
                  </span>
                ...
                <div className={solo ? "p-6" : "p-4"}>
                  <div className={`font-display text-ink group-hover:text-photon ${solo ? "text-3xl" : "text-xl"}`}>
                    {e.title}
                  </div>
                  <div className="mt-1 text-xs text-dim">by {e.author}</div>
                  {e.scores && <ScoreBars scores={e.scores} className={solo ? "mt-5 max-w-sm" : "mt-4"} />}
                </div>
              </Link>
            );
          })}
